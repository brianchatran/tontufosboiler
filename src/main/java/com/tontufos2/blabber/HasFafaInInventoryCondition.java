...
public class HasFafaInInventoryCondition implements LootCondition {
    ...
    public boolean test(LootContext context) {
        ...
        if (entity instanceof PlayerEntity player) {
            for (ItemStack stack : player.getInventory().main) {
                if (!stack.isEmpty() && stack.getItem().getTranslationKey().equals("item.tontufos2.fafa")) {
    ...
    public static void register() {
        Registry.register(Registries.LOOT_CONDITION_TYPE, new Identifier("tontufos2", "has_fafa_in_inventory"), TYPE);
    }
    ...
    public static class Serializer implements JsonSerializer<HasFafaInInventoryCondition> {
         ...
         * @return Nueva instancia de HasFafaInInventoryCondition.
         */
        @Override
